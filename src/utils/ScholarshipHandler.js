...
const createScholarship = (scholarship) => {
  ...
  axios.post('https://scholarshipandcourse.rizqisyahrendra.tech/v1/scholarships', formData, {
...
const updateScholarship = (scholarship, id) => {
  ...
  axios.patch(`https://scholarshipandcourse.rizqisyahrendra.tech/v1/scholarships/${id}`, formData, {
    withCredentials: true,
    headers: {
      'content-type': 'multipart/form-data',
    },
...
const deleteScholarship = (scholarship) => {
  ...
  .then((willDelete) => {
    if (willDelete) {
      axios.delete(`https://scholarshipandcourse.rizqisyahrendra.tech/v1/scholarships/${scholarship._id}`, {
        withCredentials: true,
      })
      .then((response) => {
        if (response.status === 200) {
...
const getDetailScholarship = async (id, setScholarship) => {
  const response =  await axios.get(`https://scholarshipandcourse.rizqisyahrendra.tech/v1/scholarships/${id}`);
  return response.data.data;
}

const getAllScholarship = async (category, counter) => {
  const response = await axios.get(`https://scholarshipandcourse.rizqisyahrendra.tech/v1/scholarships?category=${category}&page=${counter}&perPage=9`);
